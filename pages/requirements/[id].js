...
export default function Requirement({ requirement, allEstimates, allNotes }) {
  const { data: session } = useSession();
  const [formData, setFormData] = useState({});
  const [notes, setNotes] = useState(allNotes);
  const [estimates, setEstimates] = useState(allEstimates);

  async function saveNote(e) {
    e.preventDefault();
    e.target.reset();
    let dataObj = { formData, id: requirement.id };
  ...
  async function saveEstimate(e) {
    ...
    const response = await fetch("../api/AddEstimate", {
      method: "POST",
      body: JSON.stringify(dataObj),
    });
    return await response.json();
  ...
  return (
    <div className={styles.background}>
      <Head>
        <title>Slingshot - {requirement.content}</title>
      ...
      {!session ? (
        <h1 className="block mb-2 text-sm font-large text-gray-900 dark:text-gray-300 text-center">
          To Add Notes and Estimates, please Sign In!
        </h1>
      ) : (
        <>
        <>
          <div className="flex justify-center">
            <h1 className="text-white text-xl">{requirement.content}</h1>
          </div>
         <br />
          <div className="flex justify-center">
            <form onSubmit={saveNote}>
              ...
              <div class="mb-6">
                <label
                  for="project-title"
                  class="text-center block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300"
                >
                  Notes
                </label>
                <textarea
          ...
          <div
            class="text-center w-80 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative"
            role="alert"
          >
            <strong class="font-bold">Alert! </strong>
            <span class="block sm:inline">
              A note can not be edited or deleted after created
            </span>
      ...
      <h1 className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300 text-center">
        List of Notes
      </h1>
      <div className="flex justify-center">
          ...
          <ul className="list-disc">
            {estimates.map((item) => (
              <li className="m-5" key={item.id}>
                ...
                <button
                  onClick={async () => {
                    await fetch("/api/DeleteEstimate", {
                      method: "DELETE",
                      body: JSON.stringify(item.id),
                    });
                    setEstimates(estimates.filter((prj) => prj !== item));
                  }}
